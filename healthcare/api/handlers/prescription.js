 ...
 */
const getPrescription = async (event) => {
    ...
    case QrCodeTypes.PRESCRIPTION_QR:
      switch (state) {
        case PrescriptionStates.PRESCRIPTION_CREATED:
          const pharmacies = contacts.filter(contact => contact.type === ContactRoles.PHARMACIST)
          return response({state, clinic, patient, pharmacies})
        default:
 ...
 */
const getPrescriptionPrepare = async (event) => {
 ...
 */
const getPrescriptionReady = async (event) => {
 ...
 */
const getPrescriptionPickupQr = async (event) => {
...
const validatePhone = (phone, description) => {
  ...
  if (qrCode.intentState.qrType === QrCodeTypes.PRESCRIPTION_QR) {
    ...
    switch (state) {
      case PrescriptionStates.PRESCRIPTION_CREATED:
        ...
        const pharmacy = contacts.filter(contact => contact.type === ContactRoles.PHARMACIST && contact.cellPhone === pharmacyPhone)[0]
        if (!pharmacy) {
