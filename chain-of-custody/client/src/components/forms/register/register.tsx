...
import {Input, Stack, InputGroup, Textarea, Button, useToast, Select, FormLabel, Center} from '@chakra-ui/react'
...
const Register = ({bag, setBag}: RegisterProps) => {
  ...
  const onSubmit = async (data: formProps) => {
    ...
    try {
      ...
      toast({
        title: 'Thank you!',
        description: 'Your data has been submitted to the Deveron Cloud, the farmer has been notified of this',
        status: 'success',
        duration: 4000,
        isClosable: true,
      })
    ...
    <form onSubmit={handleSubmit(onSubmit)}>
      <Center direction="column" width="100%" margin="auto">
        <Stack spacing={4} maxW="500px" w="100%" padding="20px">
          ...
          <InputGroup>
            <Select
              placeholder={loadingCollectors ? 'Loading...' : 'Select Collector'}
              variant="filled"
              {...register('assetCollector')}
              isRequired
            >
              {collectors.map((e: any, i: number) => (
                <Option key={i} value={e}>{`${e.firstName} ${e.lastName}`}</Option>
              ))}
            </Select>
          </InputGroup>
          ...
          <InputGroup marginRight="10px">
            <Input bgColor="white" {...register('assetLocation')} placeholder="" value={location} />
          </InputGroup>

          <FormLabel>Notes</FormLabel>
          <InputGroup>
            <Textarea resize="vertical" size="md" bgColor="white" {...register('notes')} placeholder="" />
          </InputGroup>
          ...
          <InputGroup>
            <Select variant="filled" {...register('assetPlantingCrop')} placeholder="Select A Crop" isRequired>
